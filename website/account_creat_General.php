<?php 
	...
	if(isset($_POST["submit"]) && $emailE==0 && $nameE==0 && $phoneE==0 && $passE==0 && $dobE==0 && $addrE==0 ){
		...
		*/
		...
		$active=1;
		//---------must be transaction--------------
		$sql="insert into user VALUES (:id, :email, :password, :utype, :name, :phone, :addr, :dob,:active)";

		$stmt=$pdo->prepare($sql);

		$stmt->
		execute(['id'=>$id,'email'=>$email,'password'=>$hashed_password,'utype'=>$utype,'name'=>$name,'phone'=>$phone,'addr'=>$addr,'dob'=>$dob,'active'=>$active]);
