<?php 
  ...
  if(isset($_POST['submit'])){
    ...
    if($pwd !="" && $email!="" && filter_var($_POST['txt_email'], FILTER_VALIDATE_EMAIL) && preg_match($rexPass,$pwd) ){
      ...
      $sql="insert into hr_man values (:name, :email, :password, :phone)";
      $stmt=$pdo->prepare($sql);
      $stmt->execute(['name'=>$name,'email'=>$email, 'password'=>$hash_pwd, 'phone'=>$phone]);
  ...
  <h4 style = "text-align: center!important;"> Please fill in the required details below.. </h4>
    ...
    <div class="col" style = "width:60%">
      ...
      <div class="row edit">
        ...
        <input type="text" id="defaultRegisterFormPhone" name = "txt_phone" pattern = "[5][0-9]{7}" title = "Should start with 5 and not exceed a length of 8 digits" class="form-control mb-4" placeholder="5XXXXXXX">
      </div>
